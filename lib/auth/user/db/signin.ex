defmodule Rivet.Data.Auth.User.Db.Signin do
  ...
  # def update_user(x, y), do: Auth.User.update(x, y)
  #
  #
  ...
  ##############################################################################
  @failed_change_template Application.compile_env!(:rivet_data_auth, :user_notify_failed_change)
  @verification_template Application.compile_env!(:rivet_data_auth, :user_notify_verification)
  def add_email(user, eaddr, verified \\ false) do
    ...
    case Auth.UserEmail.one(tenant_id: user.tenant_id, address: eaddr) do
      {:ok, %Auth.UserEmail{} = email} ->
        ...
        @failed_change_template.send(email, "add email to your account.")
      ...
      {:error, _} ->
        ...
        case Auth.UserEmail.create(%{
          ...
          {:ok, %Auth.UserEmail{} = email} ->
            ...
            @verification_template.send(email)
