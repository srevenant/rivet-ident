defmodule Rivet.Data.Ident.Factor.Password do
  @moduledoc false
  def hash(password), do: Bcrypt.hash_pwd_salt(password)
  def verify(password, hash), do: Bcrypt.verify_pass(password, hash)
  def generate(), do: Rivet.Data.Ident.Factor.Password.RandChars.generate()
end

defmodule Rivet.Data.Ident.Factor.Password.Puid do
  @moduledoc """
...
defmodule Rivet.Data.Ident.Factor.Password.RandChars do
  @doc """
  random/0 generates a 72-character random string.
  """
  def generate(),
    do: Enum.map_join(1..4, "", fn _ -> Rivet.Data.Ident.Factor.Password.Puid.generate() end)
end
